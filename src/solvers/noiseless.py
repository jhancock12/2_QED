...
def to_qiskit(hamiltonian : Hamiltonian):
...
def SPSA_vqe_solver_noiseless(hamiltonian : Hamiltonian, lattice : Lattice, vqe_parameters : dict, SPSA_parameters : dict):
    if not isinstance(hamiltonian, Hamiltonian): raise TypeError(f"The Hamiltonian for measurement must be a hamiltonian_class.Hamiltonian, you have entered a {type(hamiltonian)}")
    ...
    measurer_class = CircuitMeasurer(circuit_class, hamiltonian, vqe_parameters['simulator'], vqe_parameters['shots'])
    ...
    def cost_function(thetas_values : list | np.ndarray):
        ...
        cost = measurer_class.expected_value_hamiltonian_statevector()
        return cost

    return natural_gradient_SPSA(cost_function, guess, SPSA_parameters)

def expected_value_statevector(thetas_values: list | np.ndarray, measurer_class : CircuitMeasurer):
    if not isinstance(thetas_values, list) and not isinstance(thetas_values, np.ndarray): raise TypeError(f"The values of theta must be given as a list or np.array, you have entered a {type(thetas_values)}")
    if not isinstance(measurer_class, CircuitMeasurer): raise TypeError(f"The lattice must be a measurer_class.CircuitMeasurer, you have entered a {type(measurer_class)}")

    measurer_class.bind_values(thetas_values)
    psi = qiskit.quantum_info.Statevector.from_instruction(measurer_class.parametrized_circuit)
    return np.real(psi.expectation_value(to_qiskit(measurer_class.hamiltonian)))

def electric_field_values_noiseless(thetas_values : list | np.ndarray, lattice : Lattice, vqe_parameters : dict):
    if not isinstance(thetas_values, list) and not isinstance(thetas_values, np.ndarray): raise TypeError(f"The values of theta must be given as a list or np.array, you have entered a {type(thetas_values)}")
    if not isinstance(vqe_parameters, dict): raise TypeError(f"SPSA parameters must be a dictionary, you have entered a {type(vqe_parameters)}")
    if not isinstance(lattice, Lattice): raise TypeError(f"The lattice must be a lattice_class.Lattice, you have entered a {type(lattice)}")
    ...
    for site_n in range(lattice.n_fermion_qubits):
        for direction in lattice.directions[site_n]:
            ...
            values[(lattice.labels[site_n], direction)] = expected_value_statevector(thetas_values, measurer_class)

    return values

def electric_field_values_squared_noiseless(thetas_values : list | np.ndarray, lattice : Lattice, vqe_parameters : dict):
    if not isinstance(thetas_values, list) and not isinstance(thetas_values, np.ndarray): raise TypeError(f"The values of theta must be given as a list or np.array, you have entered a {type(thetas_values)}")
    if not isinstance(vqe_parameters, dict): raise TypeError(f"SPSA parameters must be a dictionary, you have entered a {type(vqe_parameters)}")
    if not isinstance(lattice, Lattice): raise TypeError(f"The lattice must be a lattice_class.Lattice, you have entered a {type(lattice)}")
...
def magnetic_field_values_noiseless(thetas_values : list | np.ndarray, lattice : Lattice, vqe_parameters : dict):
    if not isinstance(thetas_values, list) and not isinstance(thetas_values, np.ndarray): raise TypeError(f"The values of theta must be given as a list or np.array, you have entered a {type(thetas_values)}")
    if not isinstance(vqe_parameters, dict): raise TypeError(f"SPSA parameters must be a dictionary, you have entered a {type(vqe_parameters)}")
    if not isinstance(lattice, Lattice): raise TypeError(f"The lattice must be a lattice_class.Lattice, you have entered a {type(lattice)}")
    ...
    for site_n in lattice.plaquettes:
        ...
        measurer_class.change_hamiltonian(hamiltonian)
        values[lattice.labels[site_n]] = expected_value_statevector(thetas_values, measurer_class)

    return values
    
def charge_values_noiseless(thetas_values : list | np.ndarray, lattice : Lattice, vqe_parameters : dict):
    if not isinstance(thetas_values, list) and not isinstance(thetas_values, np.ndarray): raise TypeError(f"The values of theta must be given as a list or np.array, you have entered a {type(thetas_values)}")
    if not isinstance(vqe_parameters, dict): raise TypeError(f"SPSA parameters must be a dictionary, you have entered a {type(vqe_parameters)}")
    if not isinstance(lattice, Lattice): raise TypeError(f"The lattice must be a lattice_class.Lattice, you have entered a {type(lattice)}")
...
def particle_number_values_noiseless(thetas_values : list | np.ndarray, lattice : Lattice, vqe_parameters : dict):
    if not isinstance(thetas_values, list) and not isinstance(thetas_values, np.ndarray): raise TypeError(f"The values of theta must be given as a list or np.array, you have entered a {type(thetas_values)}")
    if not isinstance(vqe_parameters, dict): raise TypeError(f"SPSA parameters must be a dictionary, you have entered a {type(vqe_parameters)}")
    if not isinstance(lattice, Lattice): raise TypeError(f"The lattice must be a lattice_class.Lattice, you have entered a {type(lattice)}")
...
def observes_reduced_noiseless(thetas_values : list | np.ndarray, lattice : Lattice, vqe_parameters : dict):
    if not isinstance(thetas_values, list) and not isinstance(thetas_values, np.ndarray): raise TypeError(f"The values of theta must be given as a list or np.array, you have entered a {type(thetas_values)}")
    if not isinstance(vqe_parameters, dict): raise TypeError(f"SPSA parameters must be a dictionary, you have entered a {type(vqe_parameters)}")
    if not isinstance(lattice, Lattice): raise TypeError(f"The lattice must be a lattice_class.Lattice, you have entered a {type(lattice)}")
    ...
    ef = electric_field_values_noiseless(thetas_values, lattice, vqe_parameters)
    ef_sq = electric_field_values_squared_noiseless(thetas_values, lattice, vqe_parameters)
    mf = magnetic_field_values_noiseless(thetas_values, lattice, vqe_parameters)
    pn = particle_number_values_noiseless(thetas_values, lattice, vqe_parameters)
    c = charge_values_noiseless(thetas_values, lattice, vqe_parameters)
    ...
    for equation in gauss_equations['equations']:
        ...
        for var in gauss_equations['independant_variables'] + lattice.gauss_equations['dependant_variables']:
            if var in list(gauss_equations['reverse_link_variable_dict']):
                sub_ins[var] = ef[gauss_equations['reverse_link_variable_dict'][var]]
            else:
                site_n_charge = int(str(var)[1:])
...
def solve_and_observe_noiseless(hamiltonian : Hamiltonian, lattice : Lattice, vqe_parameters : dict, SPSA_parameters : dict):
    if not isinstance(hamiltonian, Hamiltonian): raise TypeError(f"The Hamiltonian for measurement must be a hamiltonian_class.Hamiltonian, you have entered a {type(hamiltonian)}")
    if not isinstance(lattice, Lattice): raise TypeError(f"The lattice must be a lattice_class.Lattice, you have entered a {type(lattice)}")
    if not isinstance(SPSA_parameters, dict): raise TypeError(f"SPSA parameters must be a dictionary, you have entered a {type(SPSA_parameters)}")
