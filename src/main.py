...
# ---------- Configuration ----------
RUN_TEST = True         # Set True for a quick test (fewer points)
...
# ---------- Sweep ranges ----------
if RUN_TEST:
    g_range = np.linspace(0.3, 3.0, 3)
    bef_small = np.linspace(0.0, 1.0, 3)
    bef_large = np.linspace(0.0, 4.0, 3)
    bef_upto15 = np.linspace(0.0, 15.0, 4)
    lambda_range = np.linspace(0.0, 10.0, 3)
    x_range = np.linspace(0.5, 4.0, 3)
    e_range = np.linspace(0.0, 2.0, 3)
    n_g_list = [2, 3]               # For fixed-mass sweeps
    chiral_n_g_list = [3]           # Only n_g=3 for chiral data
else:
    g_range = np.linspace(0.3, 3.0, 15)          # 0.3, 0.493, ... , 3.0
    bef_small = np.linspace(0.0, 1.0, 9)          # 0, 0.125, ..., 1.0
    bef_large = np.linspace(0.0, 4.0, 9)          # 0, 0.5, ..., 4.0
    bef_upto15 = np.linspace(0.0, 15.0, 16)       # 0, 1, 2, ..., 15
    lambda_range = np.linspace(0.0, 10.0, 21)     # 0, 0.5, ..., 10
    x_range = np.linspace(0.5, 4.0, 8)            # 0.5, 1.0, ..., 4.0
    e_range = np.linspace(0.0, 2.0, 11)           # 0, 0.2, ..., 2.0
    n_g_list = [2, 3, 4, 5]
    chiral_n_g_list = [3]                         # Only n_g=3 for chiral
...
def build_fixed_mass_data():
    ...
    all_bef = sorted(set(bef_upto15) | set(bef_large) | set(bef_small))
...
def plot_from_data(data_fixed, data_chiral):
    ...
    for n in ng_list:
        ...
        for g in g_range:
            ...
            E_vac = get_energy(n, g, 0.0, False)
            V.append(E_charge - E_vac if not np.isnan(E_charge) else np.nan)
        V_data.append(V)
    nice_scatter_plotter(
        data_x_line=[g_range.tolist()]*len(ng_list),
        data_y_line=V_data,
        label_x=r"$g$", label_y=r"$V(g)$",
        save=True, label_save_title="V_mixed_ng",
        labels_line=[f"$n_g={n}$" for n in ng_list],
        show=False, square=False
    )
    ...
    for bef in bef_vals:
        ...
        for g in g_range:
            ...
            E_vac = get_energy(3, g, bef, False)
            V.append(E_charge - E_vac if not np.isnan(E_charge) else np.nan)
        V_large.append(V)
    nice_scatter_plotter(
        ...
        data_y_line=V_large,
        label_x=r"$g$", label_y=r"$V(g)$",
        save=True, label_save_title="V_BEF_bigger",
        labels_line=[f"$\\mathcal{{E}}={b:.1f}$" for b in bef_vals],
        show=False, square=False
    )

    # ---------- 3. Potential n_g=2, small BEF (Fig.17) ----------
    bef_small_2 = [b for b in bef_small if b < 0.5]
    V_small_2 = []
    for bef in bef_small_2:
        V = []
        for g in g_range:
            E_charge = get_energy(2, g, bef, True)
            E_vac = get_energy(2, g, bef, False)
            V.append(E_charge - E_vac if not np.isnan(E_charge) else np.nan)
        V_small_2.append(V)
    nice_scatter_plotter(
        ...
        label_x=r"$g$", label_y=r"$V(g)$",
        save=True, label_save_title="V_BEF_2",
        labels_line=[f"$\\mathcal{{E}}={b:.2f}$" for b in bef_small_2],
        show=False, square=False
    )

    # ---------- 4. Potential n_g=3, small BEF (Fig.18) ----------
    bef_small_3 = [b for b in bef_small if b < 1.0]
    V_small_3 = []
    ...
    nice_scatter_plotter(
        ...
        label_x=r"$g$", label_y=r"$V(g)$",
        save=True, label_save_title="V_BEF_2_bigger",
        labels_line=[f"$\\mathcal{{E}}={b:.2f}$" for b in bef_small_3],
        show=False, square=False
    )

    # ---------- 5. rho vs BEF (Fig.19) ----------
    for idx, g in enumerate([1.0, 2.0, 3.0]):
        rho_data = []
        ...
        nice_scatter_plotter(
            ...
            save=True, label_save_title=f"s_3_{idx}",
    ...
    for idx, g in enumerate([1.0, 2.0, 3.0]):
        ...
        for n in ng_plot:
            dH = []
            for bef in bef_upto15:
                E_n = get_energy(n, g, bef, False) if (n, g, bef, False) in data_fixed else np.nan
                E_nm1 = get_energy(n-1, g, bef, False) if (n-1, g, bef, False) in data_fixed else np.nan
        ...
        nice_scatter_plotter(
            ...
            save=True, label_save_title=f"e_3_{idx}",
            labels_line=[f"$n_g={n}$" for n in ng_plot],
            show=False, square=False
        )

    # ---------- 7. sigma and g* (Fig.21,22) ----------
    # Use threshold method as in your snippet
    THRESHOLD = 2.0
    ...
    def sigma_from_V(n_g, bef):
        ...
        for g in g_range:
            ...
            E_vac = get_energy(n_g, g, bef, False)
            V = E_charge - E_vac if not np.isnan(E_charge) else np.nan
            if np.isnan(V):
                continue
            g_vals.append(g)
            V_vals.append(V)
    ...
    nice_scatter_plotter(
        ...
        save=True, label_save_title="gs_ng",
        labels_line=[f"$n_g={n}$" for n in n_g_list],
        show=False, square=False
    )

    # ---------- 8. Delta energies (Fig.23) ----------
    for idx, g in enumerate([1.0, 2.0, 3.0]):
        dH_charges = []
        dH_vac = []
        dH_pairs = []
        for bef in bef_small:
            E_ch = get_energy(3, g, bef, True)
            E_ch0 = get_energy(3, g, 0.0, True)
            E_v = get_energy(3, g, bef, False)
            E_v0 = get_energy(3, g, 0.0, False)
    ...
    nice_scatter_plotter(
        ...
        save=True, label_save_title="chiral_initial",
    ...
    nice_scatter_plotter(
        ...
        save=True, label_save_title="chiral_initial_2",
        labels_line=[f"$x={x:.1f}$" for x in x_vals_used],
        show=False, square=False
    )

    # ---------- 10. Shift crossover (Fig.25) ----------
    # Combine all n_g (but only n_g=3 is computed; you can extend if needed)
    g_fixed = np.sqrt(2.5 / A)
    shift_data = []
    for n in n_g_list:
        # Only n=3 has data; others will be all NaN
        lam_stars = []
        for e_val in e_range:
            lam_scan = np.linspace(-0.5, 0.5, 11)
            C_scan = []
    ...
    nice_scatter_plotter(
        ...
        save=True, label_save_title="shifting_shift",
        labels_line=[f"$n_g={n}$" for n in n_g_list],
        show=False, square=False
    )
    ...
    if C_all:
        nice_scatter_plotter(
            ...
            data_y_line=C_all,
            label_x=r"$\lambda$", label_y=r"$\mathcal{C}$",
            save=True, label_save_title="chiral_small_bef",
        ...
        if chi_all:
            nice_scatter_plotter(
                ...
                data_y_line=chi_all,
                label_x=r"$\lambda$", label_y=r"$\chi_{\mathcal{C}}$",
                save=True, label_save_title="chiral_chi_small_bef",
                labels_line=[f"$\\mathcal{{E}}={b:.3f}$" for b in bef_vals],
                show=False, square=False
            )

    # 4x2 lattice (if data exists)
    # Check if we have 'chiral_bef_4x2' keys
    has_4x2 = any('chiral_bef_4x2' in key for key in data_chiral.keys())
    if has_4x2:
        C_all_4x2 = []
        chi_all_4x2 = []
        bef_vals_4x2 = []
        ...
        if C_all_4x2:
            nice_scatter_plotter(
                ...
                label_x=r"$\lambda$", label_y=r"$\mathcal{C}$",
                save=True, label_save_title="chiral_medium_bef",
            ...
            if chi_all_4x2:
                nice_scatter_plotter(
                    ...
                    label_x=r"$\lambda$", label_y=r"$\chi_{\mathcal{C}}$",
                    save=True, label_save_title="chiral_chi_medium_bef",
                    labels_line=[f"$\\mathcal{{E}}={b:.3f}$" for b in bef_vals_4x2],
                    show=False, square=False
                )
